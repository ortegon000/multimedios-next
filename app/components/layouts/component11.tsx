...
export default function Component11 ({ images, title, subtitle = 'PUBLICIDAD EXTERIOR', description }:{images:any[], title: string, subtitle?: string, description:string}): React.JSX.Element {
  return (
    <section className='w-full px-2 max-w-4xl mx-auto mt-4'>
      ...
      <div className='mt-4 px-12 flex flex-col items-center md:flex-row md:px-2 gap-2'>
        {
            images.map((item, key) => (
              <div key={key} className=''>
                <Atropos className='mx-auto'>
                  <NextImage src={item} alt='' className='rounded-md overflow-hidden mx-auto hover:bg-red-300 transition-colors' />
                </Atropos>
              </div>
            ))
          }
